...
def establish_tcp_connection(proxy_url):
    ...
    # Use create_connection for proper DNS resolution and timeout handling
    sock = socket.create_connection(connect_args, timeout=MAX_TIMEOUT)

    # Wrap with a secure context when using TLS (SNI-enabled)
    if proxy_url.scheme == "https":
        if VERIFY_TLS:
            ctx = ssl.create_default_context()
        else:
            # For testing against self-signed certs
            ctx = ssl._create_unverified_context()
        sock = ctx.wrap_socket(sock, server_hostname=proxy_url.hostname)
...
def get_upgrade_response(connection, proxy_url):
    data = b''
    # Read until end of headers or connection closed
    while b'\r\n\r\n' not in data:
    ...
    if b'\r\n\r\n' not in data:
        print("[INFO] Failed to read upgrade response: " + proxy_url.geturl())
        return None, False

    headers, rest = data.split(b'\r\n\r\n', 1)

    # Parse status line robustly (e.g., 'HTTP/1.1 101 Switching Protocols')
    try:
        ...
        print("[INFO] Failed to parse upgrade response: " + proxy_url.geturl())
        return None, False
...
def sendData(h2_connection, connection, data, stream_id):
    ...
    # `connection` is a socket-like object; use sendall to transmit bytes
    connection.sendall(h2_connection.data_to_send())


def sendSmuggledRequest(h2_connection, connection,
                        smuggled_request_headers, args, target_url):
